            ...
            <div class="row">
                <div class="col-8 mx-auto text-center">
                    ...
                    <ul class="list-inline breadcrumbs text-capitalize" style="font-weight:500">
                        ...
                        <li class="list-inline-item">/ &nbsp; <a href="{{ route('blogs') }}" wire:navigate>Blog</a>
                        </li>
                    </ul>
                </div>
            </div>
                            ...
                            <div class="col-md-6" data-aos="fade">
                                <article class="blog-post">
                                    <div class="post-slider slider-sm rounded">
                                        <img loading="lazy" decoding="async" src="{{ url('storage/'.$blog->image) }}" alt="{{ $blog?->title }}">
                                    </div>
                                    <div class="pt-4">
                                        <p class="mb-3">{{ \Carbon\Carbon::parse($blog?->created_at)->format('d M, Y') }}</p>
                                        <h2 class="h4"><a class="text-black" href="{{ route('blog',[$blog?->slug]) }}">{{ $blog?->title }}</a></h2>
                                        <p>{{ Str::limit($blog?->short_description,50,'...') }}</p> <a href="{{ route('blog',[$blog?->slug]) }}" class="text-primary fw-bold" aria-label="Read the full article by clicking here">Read More</a>
                                    </div>
                                </article>
                            </div>
                            @empty
                    ...
                    <div class="widget widget-categories">
                        ...
                        <ul class="list-unstyled widget-list">		
                            @forelse($categories as $category)	
                                <li><a href="#!">{{ $category?->name }} <small class="ml-auto">({{ $category?->blogs?->count()}})</small></a></li>			
                            @empty
                            @endforelse
                        </ul>
                    </div>
                    ...
                    <div class="widget">
                        ...
                        <ul class="list-unstyled widget-list">
                            <li class="d-flex widget-post align-items-center">
                                <a class="text-black" href="{{ route('blog',[$blog?->slug]) }}">
                                    <div class="widget-post-image flex-shrink-0 me-3">
                                        <img class="rounded" loading="lazy" decoding="async" src="{{ url('storage/'.$blog?->image ) }}" alt="Post Thumbnail">
                                    </div>
                                </a>
